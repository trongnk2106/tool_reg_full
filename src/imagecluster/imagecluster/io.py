...
def exif_timestamp(filename, dist_img):
    ...
    # for filename, img in file.items():
    img = dist_img[filename]
...
def timestamp(filename, dist_img , source='auto'):
    ...
    if source == 'auto':
        try:
            return exif_timestamp(filename, dist_img)
...
def _timestamp_worker(filename,dist_img , source):
    try:
        print("filename----", filename)
        return filename, timestamp(filename,dist_img, source)
    except OSError as ex:
        print(f"skipping {filename}: {ex}")
        return filename, None
...
def read_images(imagedir, size, ncores=mp.cpu_count()):
    ...
    """
    _f = functools.partial(_image_worker,dist_img = imagedir , size=size)
    with mp.Pool(ncores) as pool:
        ret = pool.map(_f, imagedir)
...
def read_timestamps(imagedir, source='auto', ncores=mp.cpu_count()):
    ...
    _f = functools.partial(_timestamp_worker, dist_img = imagedir, source=source)
    with mp.Pool(ncores) as pool:
        ret = pool.map(_f, imagedir)
    return {k: v for k,v in ret if v is not None}
